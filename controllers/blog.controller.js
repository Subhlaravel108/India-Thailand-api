...
const { createBlogSchema } = require("../validators/blog.validator");
...
exports.createBlog = async (req, reply) => {
  try {
    ...
    // Validation
    try {
      await createBlogSchema.validate(body, { abortEarly: false });
...
exports.updateBlogBySlug = async (req, reply) => {
  try {
...
exports.getBlogDetailsBySlug = async (req, reply) => {
    try {
        const { slug } = req.params;
        const db = req.mongo?.db || req.server?.mongo?.db;
        if (!db) {
...
exports.getAllBlogs = async (req, reply) => {
  try {
    const db = req.mongo?.db || req.server?.mongo?.db;
    if (!db) {
      return reply.code(500).send({
...
exports.deleteBlogById = async (req, reply) => {
    try{
        const {id}=req.params;
        const db = req.mongo?.db || req.server?.mongo?.db;
        if(!db){
